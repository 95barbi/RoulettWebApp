...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected UserDetailsService userDetailsService() {
		...
		// custom users
		result.createUser(User.withDefaultPasswordEncoder().username("admin").password("1234").authorities("ROLE_ADMIN").build());
		result.createUser(User.withDefaultPasswordEncoder().username("user").password("1234").authorities("ROLE_USER").build());
		return result;
	}
}
